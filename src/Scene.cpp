...
{
    // Pre-draw sequence: assign uniforms that are the same for all Geometry::draw
    // call.  These uniforms include the camera view, proj, and the lights.  These uniform
    // do not include modelview and material parameters.
    ...
    mat4 cur_VM = camera->view; // modelview at the "world" node is just the view matrix.
    ...
    for (std::map<std::string, Node *>::iterator i_node = node.begin(); i_node !=
                                                                        node.end();
         i_node++)
    {
    ...
    while (!dfs_stack.empty())
    {
        ...
        for (unsigned int i = 0; i < cur->models.size(); i++)
        {
            ...
            // Remember to set the uniform variables
            shader->setUniforms();
